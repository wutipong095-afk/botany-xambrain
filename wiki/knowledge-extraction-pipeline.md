...
**CI (GitHub Actions):** ทุก PR → job `check-text` รัน `scripts/check-text.py` อัตโนมัติ · มาตรฐาน Git ดู `CONTRIBUTING.md`
...
| ขั้น | คำสั่ง / ระบบ | เมื่อไหร่ |
|------|--------|----------|
| ตรวจโครงสร้าง vault | `ตรวจสอบ` | ก่อน merge / เป็นระยะ |
| ตรวจอักษร (CI) | GitHub Actions `check-text` | ทุก PR อัตโนมัติ |
| บันทึก git | PR → `master` (ไม่ push ตรง) | จบ session · หลังแก้ typo แล้ว |
...
| `.github/workflows/ci.yml` | CI รัน check-text บน PR |
| `CONTRIBUTING.md` | มาตรฐาน branch / commit / PR |
